...
exports.createAccountLink = async (req, res) => {
  ...
  // ✅ Case 1: Has Stripe account but onboarding not completed
  ...
  // ✅ Case 3: Already onboarded → Allow managing bank details
  if (user.stripe_account_id && user.stripe_account_status === '3') {
  const loginLink = await stripe.accounts.createLoginLink(user.stripe_account_id);

  return res.status(200).json({
    status: true,
    message: 'Manage bank details link generated successfully.',
    data: {
      type: 'manage',
      account_url: loginLink.url,
    },
  });
}

  return res.status(200).json({ status: true, message: 'Unexpected state.' });
} catch (error) {
