...
exports.createVehicleModel = async (req, res) => {
  ...
  if (!make_id || !name) {
    return res.status(400).json({ status: false, message: "make_id and name are required" });
  }
...
// ✅ Get all vehicle models (with make name)
exports.getAllVehicleModels = async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT vm.id, vm.name, vm.make_id, vm.created_at, vm.updated_at, 
...
exports.getVehicleModelById = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0)
      return res.status(404).json({ status: false, message: 'Vehicle model not found' });
  ...
  try {
    const result = await pool.query(
      `UPDATE vehicle_models 
       SET make_id = COALESCE($1, make_id),
...
exports.deleteVehicleModel = async (req, res) => {
  ...
  try {
    const result = await pool.query(`DELETE FROM vehicle_models WHERE id = $1 RETURNING *`, [id]);
