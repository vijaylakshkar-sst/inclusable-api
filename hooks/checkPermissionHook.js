...
exports.getCurrentAccessAPP = async (req, res, asMiddleware = false) => {
  const user_id = req.user.userId;
  ...
  try {
    ...
    // If Company → Stripe must be verified
    if (audience === "Company" && plan.stripe_account_status != 3) {
      data.plan.stripe_verified = false; 
      data.plan.stripe_message = "Your Stripe account is not verified. Please complete your Stripe setup.";

      // Add restrictions inside access map
      data.plan.features = {
        ...accessMap,
        stripeVerificationRequired: true,
        stripeMessage: "Your Stripe account is not verified. Please complete your Stripe setup.",
        canPostEvents: false,  // restrict
        maxEventPosts: 0,      // restrict
        canAccessPaidTicket: false // restrict
      };
    } else {
      data.plan.stripe_verified = true;
    }
